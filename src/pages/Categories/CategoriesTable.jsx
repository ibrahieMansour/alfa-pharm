...
const CategoriesTable = ({ categories, onView, onEdit, onDelete }) => {
    ...
    <table className="table users-table">
      <thead className="table-header">
        <tr className="table-row">
          ...
          <th className="table-head">اسم التصنيف</th>
      ...
      {loading ? (
        ...
        <tbody className="table-body">
          {categories?.map((e, i) => {
            return (
              <tr key={e?.id} className={`table-row ${e.isView ? "" : "baned"}`}>
          ...
          <FillTableRow RowCount={categories?.length} />
        </tbody>
      )}
    </table>
  );
